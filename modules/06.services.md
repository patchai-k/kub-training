...
1. Create a `db` service. Create `manifests/db-svc.yaml` with the following contents:
...
1. Delete and create the `backend`. Note that `kubectl apply` will not work for us this time, because we are updating the Pod startup command.
    ...
    > Tip: You can quickly delete many Pods by specifying the folder or wildcard, e.g. `kubectl delete -f manifests/` or `kubectl delete -f *.yaml`.

1. Create a `backend` service.  Create file `manifests/backend-svc.yaml`
