...
1. Navigate to the `kub-training/sample-app` folder.

    ```shell
    cd kub--training/sample-app
    ```

    > Note: `kub-training` repository should be cloned in the previous exercise.
...
1. Set the `IMAGE` environment variable and build the Docker image.
    ...
    * `docker build` command packages the application into a docker container. It does the following steps:
      ...
      * Reads the [Dockerfile](https://github.com/Altoros/kub-training/blob/master/sample-app/Dockerfile#L15)
